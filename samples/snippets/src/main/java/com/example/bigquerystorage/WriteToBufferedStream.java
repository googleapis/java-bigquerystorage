...
public class WriteBufferedStream {
	...
	private static class DataWriter {
		...
		// A queue that contains the list of offsets need to be flushed.
		private final BlockingQueue<Integer> flushQueue = new LinkedBlockingDeque<>(10 * 1000);
		// A separate thread that handles flush calls.
		private Thread flushThread;

		private final Object lock = new Object();
		Condition hasMessageInFlushQueue = lock.newCondition();
		...
		private void flushLoop() {
			while (!waitingFlushQueueDrained()) {
				long flushOffset = 0;
				this.lock.lock();
				try {
					hasMessageInWaitingQueue.await(100, TimeUnit.MILLISECONDS);
					Integer flushOffset = -1;
					while (!this.flushQueue.isEmpty()) {
						flushOffset = this.waitingRequestQueue.pollFirst();
					}
				} catch (InterruptedException e) {
					log.warning(
							"Interrupted while waiting for message. Stream: Error: "
									+ e.toString());
				} finally {
					this.lock.unlock();
				}
				...
				if (flushOffset >= 0) {
					FlushRowsRequest preparedRequest = FlushRowsRequest.newBuilder().setWriteStream(streamName).setOffset(
							Int64Value.newBuilder().setValue(flushOffset).build();
				}
			}
		}
		 ...
		 */
		private boolean waitingFlushQueueDrained() {
			this.lock.lock();
			try {
		...
		static class AppendCompleteCallback implements ApiFutureCallback<AppendRowsResponse> {
			...
			public void onSuccess(AppendRowsResponse response) {
				...
				if (response.getAppendResult().getOffset() == parent.getFlushOffsets()) {
					parent.flushQuque.write();
				}
				done();
			}
