...
/**
 * A profiler that would periodically generate a report for the past period with the latency report for the slowest
 * requests. This is used for debugging only.
 *
...
public class RequestProfiler {
    ...
    void startOperation(OperationName operationName, String requestUniqueId) {
        idToIndividualOperation.putIfAbsent(requestUniqueId,
                new IndividualRequestProfiler(requestUniqueId));
    ...
    String flushAndGenerateReportText() {
        ...
        String reportText = String.format("At system time %s, in total %s finished during the "
